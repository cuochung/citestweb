...
<template>
  <Loading v-if="store.state.loading" /> <!-- 讀取畫面 自已設計的loading -->
  <PDDToastMultiV2Pinia /> <!-- 讀取畫面 自已設計的 toast -->
  
  token: [{{  store.state.logined_token }}] - loading: [{{ store.state.loading }}]
  <div>
    <div class="flex justify-between items-center">
      <h1>codeigniter 4 前端樣本 (CURD,Login,Token)</h1>

      <div class="login_section flex h-20 gap-5 items-center" v-if="!store.state.logined_token.length">
